    ...
    def fit(self):
        ...
        for t in range(self.T):
            ...
            for i in range(len(dataset)):
                ...
                if(i == index and j < len(locations)):
                    self.dataset.loc[i, 'weights'] *= np.exp(alpha*Evaluation.loc[j,'misclassified'])
                    j = j + 1
                    
                    if(j < len(locations)):
                        index = locations[j]
                else:
                    self.dataset.loc[i, 'weights'] = self.dataset.loc[i, 'weights']
